...
static bool insideTriangle(int x, int y, const Vector3f* _v)
{
	...
	//// v[0]~A, v[1]~B, v[2]~C
	//Eigen::Vector3f AB(_v[1].x() - _v[0].x(), _v[1].y() - _v[0].y(), _v[1].z() - _v[0].z());
	//Eigen::Vector3f BC(_v[2].x() - _v[1].x(), _v[2].y() - _v[1].y(), _v[2].z() - _v[1].z());
	//Eigen::Vector3f CA(_v[2].x() - _v[0].x(), _v[2].y() - _v[0].y(), _v[2].z() - _v[0].z());

	//Eigen::Vector3f AP(point.x() - _v[0].x(), point.y() - _v[0].y(), point.z() - _v[0].z());
	//Eigen::Vector3f BP(point.x() - _v[1].x(), point.y() - _v[1].y(), point.z() - _v[1].z());
	//Eigen::Vector3f CP(point.x() - _v[2].x(), point.y() - _v[2].y(), point.z() - _v[2].z());
	...
	if (MSAA)
	{
		...
		for (int x = min_x; x <= max_x; x++)
		{
			for (int y = min_y; y <= max_y; y++)
			{
				...
				// number of point inside triangle
				int count = 0;
				// if four point in triangle
				for (int i = 0; i < 4; i++)
				{
					if (insideTriangle((float)x + pos[i][0], (float)y + pos[i][1], t.v))
					{
						// if in, lerp with depth z
